...
for year in range(2017, 2018):
    for month in range(12, 13):
        ...
        for day in range(31, month_range + 1):
            result_return = []
            result_risk = []
            result_sub = []
            ...
            try:
                io.to_sql("fund_weekly_return", conn=engine_wt, dataframe=df_return, chunksize=500)
                io.to_sql("fund_weekly_risk", conn=engine_wt, dataframe=df_risk, chunksize=500)
                io.to_sql("fund_weekly_subsidiary", conn=engine_wt, dataframe=df_sub, chunksize=500)
