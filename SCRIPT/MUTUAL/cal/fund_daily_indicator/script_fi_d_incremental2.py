...
for update_time in [UPDATE_TIME]:
    ...
    for statistic_date, ids_used in sorted(tasks.items(), key=lambda x: x[0]):
        ...
        df_3.columns = cols_3
        try:
            io.to_sql("fund_daily_risk2", conn=engine_wt, dataframe=df_1, chunksize=500)
            io.to_sql("fund_daily_subsidiary2", conn=engine_wt, dataframe=df_2, chunksize=500)
            io.to_sql("fund_daily_subsidiary3", conn=engine_wt, dataframe=df_3, chunksize=500)
