...
for year in range(2017, 2018):
    for month in range(2, 3):
        ...
        for day in range(25, month_range + 1):
            result_return = []
            result_risk = []
            result_sub = []
            ...
            try:
                io.to_sql("fund_daily_return", conn=engine_wt, dataframe=df_return, chunksize=500)
                io.to_sql("fund_daily_risk", conn=engine_wt, dataframe=df_risk, chunksize=500)
                io.to_sql("fund_daily_subsidiary", conn=engine_wt, dataframe=df_sub, chunksize=500)
