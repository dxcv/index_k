...
for statistic_date, ids_used in sorted(tasks.items(), key=lambda x: x[0]):
    ...
    df_3.columns = cols_3
    try:
        io.to_sql("fund_monthly_return", conn=engine_wt, dataframe=df_1, chunksize=500)
        io.to_sql("fund_monthly_risk", conn=engine_wt, dataframe=df_2, chunksize=500)
        io.to_sql("fund_monthly_subsidiary", conn=engine_wt, dataframe=df_3, chunksize=500)
