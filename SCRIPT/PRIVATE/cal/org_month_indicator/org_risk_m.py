...
columns.extend(table.columns()[:-16])
columns.extend(table.columns()[-8:])
...
def calculate():
    ...
    for i in range(len(ids_o)):
        ...
        mdd_ = [fi.max_drawdown(nv) for nv in navs2]
        mdd = [x[0] if x is not None else None for x in mdd_]
        mdd_time = [x[1][0] if x[1] is not None else None for x in mdd_]
        VaR = [fi.value_at_risk(r) for r in rs2]
        ...
        for k in benchmark.keys():
            ...
            tmp = [stdev_a, dd_a, beta, corr, mdd, mdd_time, VaR]
            result_i = [ids_o[i], names_o[i], k, 1, 1, nv_matrix.shape[1], 60001, "全产品", 6000101, "全产品", date_s.today]
            for x in tmp:
                result_i.extend(x)
            result.append(result_i)

    df = pd.DataFrame(result)
    df[list(range(11, 67))] = df[list(range(11, 67))].astype(np.float64)
    df[list(range(11, 67))] = df[list(range(11, 67))].apply(lambda x: round(x, 6))
    df.columns = columns
