...
def calculate():
    ...
    t_mins_pe_all = {k: dt.datetime(x - 1, 12, 31) for (k, x) in t_mins_pe_all.items()}
    pesid_used = []
    ...
    for idx in pesid_used:
        ...
        pe["statistic_date"] = pe["statistic_date"].apply(lambda x: su.date2tstp(x) - 864000)
    ...
    for mday in range(7, 8):
        ...
        ts_std_total = [
            tu.timeseries_std(date_s.today, tu.periods_in_interval(date_s.today, t_min, 12), periods_y=12,
        ...
        # 基准指数的标准序列_成立以来
        matchs = [tu.outer_match4indicator_m(t, t_std_all, False) for t, t_std_all in zip(ts, ts_std_total)]
        ...
        for i in range(len(index_used)):
            ...
            dd_a = [fi.downside_deviation_a(r, r_f, 12) for r, r_f in zip(r_all3, r_f_all3)]
            mdd = [fi.max_drawdown(price)[0] for price in price_all3]
            sharpe_a = [fi.sharpe_a(r, r_f, 12) for r, r_f in zip(r_all3, r_f_all3)]
            calmar_a = [fi.calmar_a(price, r_f, 12) for price, r_f in zip(price_all3, r_f_all3)]
            sortino_a = [fi.sortino_a(r, r_f, 12) for r, r_f in zip(r_all3, r_f_all3)]
            ...
            con_rise_months = [fi.periods_continuous_rise(r)[0] for r in r_all3]
            con_fall_months = [fi.periods_continuous_fall(r)[0] for r in r_all3]

            tmp = [ir, ir_a, stdev_a, dd_a, mdd, sharpe_a, calmar_a, sortino_a, p_earning_months, n_earning_months,
                   con_rise_months, con_fall_months]
    ...
    df[list(range(3, 101))] = df[list(range(3, 101))].apply(lambda x: round(x, 6))
    df.columns = columns
